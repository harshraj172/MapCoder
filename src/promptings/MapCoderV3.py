...
""",
        ...
        response = self.trim_text(
            response, f"# Let's think step by step to solve this problem in Python3 programming language.")
        ...
        for planning_with_ex in plannings:
            ...
            for i in range(1, self.t + 1):
                print(f"Changing code language: from Python3 to {self.language}")
                input_for_changing_code_language = [
                    {
                        "role": "user",
                        "content": (
                            f"Given a code snippet written in Python 3, your task is to translate it into {self.language} while preserving the variable names "
                            f"and the logic of the original code. Do not change any functionality; only convert the syntax to the target language. "
                            f"Ensure that the translated code is enclosed within code blocks specific to {self.language}. For example, produce the code as ```{self.language}\n<code_in_target_language>\n```\n\n"
                            f"## Python 3 Code:\n"
                            f"{code}\n\n"
                            f"## {self.language.upper()} Code:\n"
                        )
                    }
                ]

                print("\n\n________________________")
                print("Input for changing code language: ")
                print(input_for_changing_code_language[0]['content'], flush=True)

                response_in_target_language, pr_tok_1, com_tok_1 = self.gpt_chat(
                    input_for_changing_code_language
                )
                item['api_calls'] += 1
                # time.sleep(1)

                code_in_target_language = self.parse_code(response_in_target_language)
                pr_tok += pr_tok_1
                com_tok += com_tok_1

                print("\n\n________________________")
                print("Response from changing code language: ")
                print(response_in_target_language, flush=True)
                ...
                if i == 1:
                    input_for_improving_code = [
                        {
                            "role": "user",
                            "content": f"Given a competitive programming problem you have generated Python3 code to solve the problem. But the generated code can not pass sample test cases. Improve your code to solve the problem correctly.\n{algorithm_prompt}\n## Problem to be solved:\n{self.data.get_prompt(item)}\n{response}\n## Test Report:\n{test_log}\n## Modified Planning:\n## Let's think step by step to modify Python3 Code for solving this problem.\n\n----------------\nImportant:\n{std_input_prompt}\n## Your response must contain the modified planning and then the Python3 code inside ``` block to solve this problem."
                        }
                    ]
                else:
                    input_for_improving_code.append(
                        {
                            "role": "user",
                ...
                print(input_for_improving_code[-1]['content'], flush=True)

                response, pr_tok_2, com_tok_2 = self.gpt_chat(
                    input_for_improving_code
                )
                input_for_improving_code.append({"role": "assistant", "content": response})
                item['api_calls'] += 1
                # time.sleep(1)
        ...
        return code_in_target_language, pr_tok, com_tok
